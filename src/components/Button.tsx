...
const Btn = React.memo(({ title, onPress, disabled, children, className, ...rest }: ButtonProps) => {
  return (
    <TouchableOpacity
      ...
      {...rest}
    >
      <SemiBold style={{ fontSize: MAIN_BUTTON_SIZE, paddingBottom: 2 }} className='text-center text-white'>
        {title || children}
      </SemiBold>
    </TouchableOpacity>
  )
...
export const BtnTransparent = React.memo(({ title, onPress, children, style }: ButtonProps) => {
  return (
    ...
    >
      <SemiBold style={[{ fontSize: MAIN_BUTTON_SIZE }, style]} className='text-center text-accent'>
        {title || children}
      </SemiBold>
    </TouchableOpacity>
  )
