...
import type { Chart } from '../../lib/fcfs'
        ...
        <View className='flex-row'>
          ...
          <View className='gap-y-2 pl-2'>
            {chart.map((item, index) => (
              ...
              >
                <Medium style={{ fontSize: 11 }}>{item.startTime}</Medium>
              </View>
            ))}
            <View className='w-full items-center'>
              <Medium style={{ fontSize: 11 }}>{maxTime}</Medium>
            </View>
          </View>
        </View>
        <View className='flex-row justify-between'>
          <View className='items-center justify-center gap-y-2 pl-2'>
            <Medium className='text-xs'>
              Avg. Waiting Time: {(chart.reduce((acc, item) => acc + item.startTime, 0) / chart.length).toFixed(2)}
            </Medium>
            <Medium className='text-xs'>
              Avg. Turnaround Time: {(chart.reduce((acc, item) => acc + item.endTime, 0) / chart.length).toFixed(2)}
            </Medium>
